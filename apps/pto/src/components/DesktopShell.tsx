...
// calendar's, which is itself the directory's minus the member search box.
// Wraps the bundle's screens only — the storefront is server-rendered and has
// its own chrome (see ROUTING.md).
    ...
    <div className={`sd sd-desktop ${locale === "zh" ? "sd-zh" : ""}`}>
      ...
      <div className="sd-deskmain">
        ...
        <header className="sd-deskhead">
          ...
          {/* Unlike the calendar's, nothing behind this shell is public, so a
              signed-out visitor never reaches it — the router has already sent
              them to /sign-in. The slot is kept for the one frame between the
              session resolving and the redirect. */}
          {!me && !loading && (
