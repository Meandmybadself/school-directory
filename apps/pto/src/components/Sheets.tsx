// Bottom sheets: the language picker and the account menu.
//
// Copied from apps/store, which copied the generic half of the calendar's — see
// CLAUDE.md, "Four front ends, one API": the design system is copied between
// apps rather than imported, and the copies are expected to drift.
//
// The language picker is here even though the board screens are English. It is
// not decoration: the account sheet is where a member changes the language that
// then follows them to the directory, the calendar and this site's own public
// page, all of which ARE translated.
...
/** Account menu. The directory app has a Person switcher here; the boards are
 *  not Person-scoped — a card names a Person, but the app acts as the ACCOUNT —
 *  so this offers the one thing that does apply: sign out. (It used to link to
 *  the directory too; the platform switcher — AppSwitcher.tsx — now does that
 *  for every sibling app.) */
