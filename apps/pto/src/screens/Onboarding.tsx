// Sign in → check email, for this app's own origin.
//
// Reached only by PTO board members and system admins: nothing else in this
// bundle is public, and the public page needs no account at all. `api.authStart`
// sends returnTo, so the magic-link callback redirects back here rather than to
// the directory — the session itself is shared either way.
//
// The response to /auth/start is always identical whether or not the address has
// an account (invariant 4, no account enumeration), so this always shows "check
// email". Note that signing in successfully still does not get anyone onto the
// boards: that is a second gate, resolved by `GET /pto/access`, and a member who
// clears the first and not the second lands on NoAccess.
  ...
  return (
    <div className="sd-row" style={{ gap: 10, justifyContent: center ? "center" : "flex-start" }}>
      <div style={{ width: 34, height: 34, borderRadius: 10, background: "var(--blue)", color: "var(--on-brand)", display: "flex", alignItems: "center", justifyContent: "center", flex: "0 0 auto" }}>
        <Icon name="table" size={19} stroke={1.9} />
      </div>
      <div style={{ lineHeight: 1.05 }}>
        <div style={{ fontSize: 15.5, fontWeight: 800, letterSpacing: "-.4px" }}>{t("brand")}</div>
        <div style={{ fontSize: 8, fontWeight: 700, letterSpacing: ".8px", textTransform: "uppercase", color: "var(--ink-3)" }}>{t("brandSubPto")}</div>
      </div>
    </div>
  );
}
      ...
      <div className="sd-scroll" style={{ display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "24px 28px 0", gap: 16, textAlign: "center", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%", marginTop: 2 }}>
            <button className="sd-btn sd-btn-ghost" onClick={() => void resend()} disabled={resent}>
