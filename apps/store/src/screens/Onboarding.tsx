...
//
// Reached almost only by admins. Buying needs no account (invariant-free by
// design: guest checkout is the whole point of a public storefront), so a
// shopper never sees this screen — a signed-in member gets their orders
// attached automatically, and a guest gets a status link by email.
// `api.authStart` sends
// returnTo, so the magic-link callback redirects back here rather than to the
// directory — the session itself is shared either way. The response to
// /auth/start is always identical (no account enumeration), so we always show
// "check email".
  ...
  return (
    <div className="sd-row" style={{ gap: 10, justifyContent: center ? "center" : "flex-start" }}>
      <div style={{ width: 34, height: 34, borderRadius: 10, background: "var(--blue)", color: "var(--on-brand)", display: "flex", alignItems: "center", justifyContent: "center", flex: "0 0 auto" }}>
        <Icon name="home" size={19} stroke={1.9} />
      </div>
      <div style={{ lineHeight: 1.05 }}>
        <div style={{ fontSize: 15.5, fontWeight: 800, letterSpacing: "-.4px" }}>{t("brand")}</div>
        <div style={{ fontSize: 8, fontWeight: 700, letterSpacing: ".8px", textTransform: "uppercase", color: "var(--ink-3)" }}>{t("brandSubStore")}</div>
      </div>
    </div>
  );
}
